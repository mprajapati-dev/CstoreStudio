  ...
  return (
    <div className="p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="space-y-6">
        ...
        {tickets.filter(j => j.status === 'AWAITING_BIDS').length === 0 && <p className="text-gray-400 text-sm">No open jobs...</p>}
        {tickets.filter(j => j.status === 'AWAITING_BIDS').map(job => (
        ...
        {tickets.filter(j => j.status === 'IN_PROGRESS').length === 0 && <p className="text-gray-400 text-sm">No assigned jobs...</p>}
        {tickets.filter(j => j.status === 'IN_PROGRESS').map(job => (
